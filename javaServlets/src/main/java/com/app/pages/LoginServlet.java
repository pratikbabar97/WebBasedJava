	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				// 8. save candidate dao instance in session scoped attr map
				session.setAttribute("candidate_dao", candidateDao);
				// 9. save admin dao instance in session scoped attr map
				session.setAttribute("admin_dao", adminDao);
				...
				if (user.getRole().equals("admin"))
					...
					// voter role
					if (user.isStatus() == true) // already voted
